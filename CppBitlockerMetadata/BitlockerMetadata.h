...
struct FVE_METADATA_HEADER 
{
	...
	BITLOCKER_STATE currState <format=hex>;
	BITLOCKER_STATE nextState <format=hex>;

	//--- Size of the encrypted volume ---//
	int64 encVolSize <format=hex>;
...
struct _FVE_DATASET
{
  unsigned int DataSetSize <format=hex>;
  unsigned int DataSetVersion;
  unsigned int DataSetStart <format=hex>;
  unsigned int DataSetEnd <format=hex>;
  GUID FveIdentification;
  unsigned int NonceCounter;
  //-----------------
  //--- Changed from unsigned __int16 to ENC_TYPES
  ENC_TYPES FvekType <format=hex>;
  ENC_TYPES FvekPrefType <format=hex>;
  //----------------//
  FILETIME DateTime;
};
...
byte    EXTERNAL_KEY    :   1;
byte    PIN             :   1;
byte    RECOVERY_PASSWD :   1;
byte    PUBLIC_KEY      :   1;
byte    PASSPHRASE      :   1;
byte    EXTERNLY_MGD    :   1; //--- Unknown what protector from Win32_EncryptableVolume::GetKeyProtectorType this refers to
byte    UKNOWN_1        :   1;    
...
struct TPM2B_SENSITIVE_DATA_DUMMY(long encSize){
    UINT16    bits_size;
...
struct TPMT_SENSITIVE_DUMMY(uint outPrivate_size){
    //---------------------------------------
    ...
    TPMU_SENSITIVE_COMPOSITE_DUMMY    sensitive(encSize);
};
...
struct TPM2B_SENSITIVE_DUMMY (uint outPrivate_size) {
    //--- Dummy TPM2B_SENSITIVE structure to work out size of encrypted data ---//
    UINT16               sensitive_size;
    TPMT_SENSITIVE_DUMMY sensitiveArea(outPrivate_size);
...
struct TPM2B_PRIVATE_DUMMY{
    ...
    //-------------------------------//
        ...
        TPM2B_SENSITIVE_DUMMY    encryptedSensitiveArea(outPrivate_size);
    };
...
struct _FVE_DATUM_TEMP
{
  ...
  if (h.Type == FveDatumKeyTypeProp){
    ENC_TYPES KeyType;
    //--- May not be the same flags as the stretch key flags ---//
    KEY_FLAGS KeyFlags;
    byte KeyData[h.DatumSize-12];
    }
  else if (h.Type == FveDatumUnicodeTypeProp){
    wchar_t UnicodeData[(h.DatumSize-8)/2];
  }
	...
	ENC_TYPES encryption_method <format = hex>;
	...
	byte salt[16] <format = hex>;
    
    //--- Nested datums as denoted by DatumTypes ---//
    local int Datums = GetNoOfDatums(h.DatumSize-28);
    _FVE_DATUM_TEMP StretchDatums[Datums] <read = READ_DATUM_TYPE,optimize=false>;
      ...
      int nonceCounter;
      byte HMAC[16] <format=hex>;
      byte AesCCMData[h.DatumSize-36];  
      //----------------------------------------//
  }
      ...
      TPM2B_PRIVATE_DUMMY outPrivateDummy;

      TPM2B_PUBLIC outPublic;
      TPM2B_DIGEST pcrDigest;
      LittleEndian();
      byte sizeofSelect <format = hex>;
      ...
      //---------------------------------------------------------//
        ...
        GUID Identifier;
        FILETIME ProtectionTime;
        WORD VmkHints;
        PROTECTOR_TYPES ProtectionType<read=READ_PROTECTOR_TYPE>;
        local int Datums = GetNoOfDatums(h.DatumSize-36);
...
struct FveHeaderBlock(int currOffset){
    ...
    //--- Only mentioned by:
    //--- https://github.com/libyal/libbde/blob/main/documentation/BitLocker%20Drive%20Encryption%20(BDE)%20format.asciidoc#5-fve-metadata-block (Metz, 2022)
    byte padding[FVEBlockHeader.size*16-FVEDataset.DataSetSize-0x40];
    //-----------------------//
};
...
//--- Uncomment for the backup metadata blocks ---//
